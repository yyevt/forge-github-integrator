...
export class Jira implements IssueTrackingSoftware {
    ...
    public async searchIssues(issueKeys: ReadonlyArray<string>): Promise<ReadonlyArray<Issue>> {
        ...
        const issueDtos = await this.jiraApi.searchJQL<JiraIssuesShortDto>({jql, fields: ["key", "summary", "status"]});
    ...
    public async retrieveIssue(issueKey: string): Promise<Issue> {
        const issueDto = await this.jiraApi.getIssue<JiraIssueShortDto>(issueKey, ["key", "summary", "status"]);
        return this.jiraMapper.mapToEntity(issueDto);
    }

    public async closeIssue(issueKey: string): Promise<void> {
        const transitionsDto = await this.jiraApi.getTransitions(issueKey);
        ...
        return this.jiraApi.doTransition(issueKey, {transition: {id: closingTransition.id}});
    }
}